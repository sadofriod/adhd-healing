...
const mainDocument = {
  title: '主报告',
  path: 'session-123/2026-main.md',
  linkTarget: 'session-123/2026-main',
} as const;

const researchDocument = {
  title: '许可证执行指南',
  path: 'session-123/2026-main--许可证执行指南.md',
  linkTarget: 'session-123/2026-main--许可证执行指南',
} as const;
...
describe('obsidian explicit wiki-link materialization', () => {
  test('rewrites explicit links to concrete document paths', () => {
    ...
    expect(markdown).toContain(
      '[[session-123/2026-main--许可证执行指南|许可证执行指南]]'
    );
    expect(markdown).toContain(
      '[[session-123/MCP-实施指南|MCP 文档]]'
    );
