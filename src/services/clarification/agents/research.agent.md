...
7. 在有语义的句子中使用 3-8 个稳定、原子化的 Obsidian `[[双链]]`，优先连接主报告、相关概念和后续执行节点，不要堆砌孤立链接。
8. 不生成 frontmatter、父报告链接、原始会话或归档尾注；归档层会统一补充。
9. 最终只输出请求的严格 JSON，不输出 Markdown 代码块、工具过程或额外解释。
